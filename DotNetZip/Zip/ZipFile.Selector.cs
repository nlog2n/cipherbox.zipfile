...
namespace CipherBox.Zip
{
    ...
    partial class ZipFile
    {
                                               ...
                                               bool wantUpdate)
        {
            ...
            CipherBox.Zip.Filter.FileSelector ff = new CipherBox.Zip.Filter.FileSelector(selectionCriteria,
...
namespace CipherBox.Zip.Filter
{
    internal abstract partial class SelectionCriterion
    {
        internal abstract bool Evaluate(CipherBox.Zip.ZipEntry entry);
    }


    internal partial class NameCriterion : SelectionCriterion
    {
        internal override bool Evaluate(CipherBox.Zip.ZipEntry entry)
        {
    ...
    {
        private bool Evaluate(CipherBox.Zip.ZipEntry entry)
        {
        ...
        public ICollection<CipherBox.Zip.ZipEntry> SelectEntries(CipherBox.Zip.ZipFile zip)
        {
            if (zip == null)
                throw new ArgumentNullException("zip");

            var list = new List<CipherBox.Zip.ZipEntry>();

            foreach (CipherBox.Zip.ZipEntry e in zip)
            {
                if (this.Evaluate(e))
                    list.Add(e);
            }
        ...
        ///
        /// <returns>a collection of ZipEntry objects that conform to the criteria.</returns>
        public ICollection<CipherBox.Zip.ZipEntry> SelectEntries(CipherBox.Zip.ZipFile zip, string directoryPathInArchive)
        {
            if (zip == null)
                throw new ArgumentNullException("zip");
