...
namespace CipherBox.Zip
{
    public partial class ZipEntry
    {
...
#if BZIP
            if (this.CompressionMethod == CipherBox.Zip.CompressionMethod.BZip2)
        ...
        private bool WantReadAgain()
        {
            ...
            if (CompressionLevel == CipherBox.Zip.Zlib.CompressionLevel.None) return false;
        ...
        private void MaybeUnsetCompressionMethodForWriting(int cycle)
        {
            ...
            if (CompressionLevel == (short)CipherBox.Zip.Zlib.CompressionLevel.None &&
                CompressionMethod == CipherBox.Zip.CompressionMethod.Deflate)
                _CompressionMethod = 0x00;
        ...
        internal void WriteHeader(Stream s, int cycle)
        {
            ...
            // LastMod
            _TimeBlob = CipherBox.Zip.SharedUtilities.DateTimeToPacked(LastModified);
        ...
        private Int32 FigureCrc32()
        {
            if (_crcCalculated == false)
            {
                ...
                {
                    var output = new CipherBox.Crc.CrcCalculatorStream(Stream.Null);
                ...
                else
                {
                    ...
                    var crc32 = new CipherBox.Crc.CRC32();
        ...
        private void _WriteEntryData(Stream s)
        {
            ...
            try
            {
                ...
                var output = new CipherBox.Crc.CrcCalculatorStream(compressor, true);
        ...
        internal void FinishOutputStream(Stream s,
                                         ...
                                         Stream encryptor,
                                         Stream compressor,
                                         CipherBox.Crc.CrcCalculatorStream output)
        {
            ...
            if ((compressor as CipherBox.Zip.Zlib.DeflateStream) != null)
                compressor.Close();
#if BZIP
            else if ((compressor as CipherBox.Zip.BZip2.BZip2OutputStream) != null)
                compressor.Close();

            else if ((compressor as CipherBox.Zip.BZip2.ParallelBZip2OutputStream) != null)
                compressor.Close();
#endif

            else if ((compressor as CipherBox.Zip.Zlib.ParallelDeflateOutputStream) != null)
                compressor.Close();
            ...
            if (_UncompressedSize == 0 && _CompressedSize == 0)
            {
                ...
                if (_Password != null)
                {
                    ...
                    if (Encryption != EncryptionAlgorithm.None)
                    {
                        ...
                        // workitem 10178
                        CipherBox.Zip.SharedUtilities.Workaround_Ladybug318918(s);
        ...
        internal void PrepOutputStream(Stream s,
                                       ...
                                       out CipherBox.Crc.CrcCalculatorStream output)
        {
            ...
            // Wrap a CrcCalculatorStream around that.
            // This will happen BEFORE compression (if any) as we write data out.
            output = new CipherBox.Crc.CrcCalculatorStream(compressor, true);
        }
        ...
        {
            if (_CompressionMethod == 0x08 && CompressionLevel != CipherBox.Zip.Zlib.CompressionLevel.None)
            {
                     ...
                     _container.ParallelDeflateThreshold > 0L))
                {
                    ...
                    if (_container.ParallelDeflater == null)
                    {
                        _container.ParallelDeflater =
                            new CipherBox.Zip.Zlib.ParallelDeflateOutputStream(s,
                                                                       CompressionLevel,
                                                                       _container.Strategy,
                                                                       true);
                    ...
                    CipherBox.Zip.Zlib.ParallelDeflateOutputStream o1 = _container.ParallelDeflater;
                    o1.Reset(s);
                    return o1;
                }

                var o = new CipherBox.Zip.Zlib.DeflateStream(s, CipherBox.Zip.Zlib.CompressionMode.Compress,
                                                     CompressionLevel,
                                                     true);
                if (_container.CodecBufferSize > 0)
            ...
            if (_CompressionMethod == 0x0c)
            {
                     ...
                     _container.ParallelDeflateThreshold > 0L))
                {
                    ...
                    var o1 = new CipherBox.Zip.BZip2.ParallelBZip2OutputStream(s, true);
                    return o1;
                }

                var o = new CipherBox.Zip.BZip2.BZip2OutputStream(s, true);
                return o;
            }
#endif
